...
%Add fiducial points
xSPM.Z = [NaN NaN NaN]; %Fiducial value
...
if render_template %rendered on a spm template
    ...
    switch sel
        case 0
            %% fcNIRS: tag channels according to the cluster they belong to
            % Compute clustergram
            [PERM1 T2 corrMat] = nirs_clustergram(dataNIRS);
            % Add channels (projected onto scalp)
            % Add channels (projected onto cortex)
            for i0 = 1: Nc
                % No projections on the scalp
                % xSPM.Z = [xSPM.Z -10 10];
                xSPM.Z = [xSPM.Z T2(i0)-1];
                xSPM.XYZmm = [xSPM.XYZmm Ch_mc1(:,i0)];
                % xSPM.label = [xSPM.label ['C' int2str(i0) '_Slp'] ['C' int2str(i0) '_Ctx']];
                xSPM.label = [xSPM.label ['C' int2str(i0) '_Ctx']];
            end
            opt.type = 'Channel';
        case 1
