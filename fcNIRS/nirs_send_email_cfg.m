...
subject.val         = {['[nirs10] [%DATE%] On behalf of ' spm('Ver')]};
subject.strtype     = 's';
subject.num         = [1 Inf];
...
message.val         = {['Hello from nirs10!' sprintf('\n') 'Job finished on %DATE%' ]};
message.strtype     = 'e';
message.num         = [1 Inf];
...
try
    ...
    '% Copyright (C) 2013 LIOM Laboratoire d''Imagerie Optique et Moléculaire' sprintf('\n') ...
    ...
    if ~isempty(job.attachments)
        if strcmpi(job.params.zip,'Yes')
            zipfile = fullfile(tempdir,'nirs_sendmail.zip');
