...
fName{1} = 'epi122_C1_Raw Data';
fName{2} = 'epi122_C2_Raw Data';
fName{3} = 'epi122_C3_Raw Data';
fName{4} = 'epi122_C4_Raw Data';
fName{5} = 'epi122_C5_Raw Data';
fName{6} = 'epi122_C6_Raw Data';
...
for iFile = 1:length(fName)
    fileName = fullfile(pathName,[fName{iFile} '.dat']);
    fid = fopen(fileName);
    fprintf('Reading %s...\n', fileName);
    for iChannels = 1:nChannels{iFile}
        hdr(iFile, iChannels) = textscan(fid,'%s', 1);
        EEGdata(iFile, iChannels) = textscan(fid, '%f', dataPoints{iFile}, 'CollectOutput', 1);
    end
