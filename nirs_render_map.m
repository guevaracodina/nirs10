function DF = nirs_render_map(F,W,Z)
try
    DF = [];
    ...
    s_map = F.s_map;
    I = [];
    I.index_over = find(s_map >= W.thz); 
    I.index_over2 = find(s_map <= -W.thz);
    %********causion: for the convenience of making movies. Show both
    %colorbars, can be removed.
    if isempty(I.index_over)
        I.index_over = 1;
        F.s_map(1) = W.thz;
    end
    if isempty(I.index_over2)
        I.index_over2 = 2;
        F.s_map(1) = -W.thz;
    end    
    %******************************************************************
    Y1 = make_render_map(I,F,W,Z,0);
    if ~isempty(Y1)
        DF.fh1 = Y1.fh1;
        DF.ax1 = Y1.ax1;
        DF.hc1 = Y1.hc1;
...
function Y = make_render_map(I,F,W,Z,combinedfig)
...
try
    ...
    if ~isempty(index_over) || (combinedfig && ~isempty(index_over2))
        ...
        [min_T max_T min_T2 max_T2 T_brain_over fcool fgray fhot sbar] = nirs_get_Tmap(F.tstr,Z.GroupColorbars,combinedfig,index_over,index_over2,...
            W.thz,min_max_gap,s_map,overwrite_map_thresholds,brain,th_z_shrink,TC);
        write_fig = 1;
    end
    if write_fig
        ...
        %fh1 = figure('Visible','on','Name',[str_cor ' ' contrast_info],'NumberTitle','off');
        ax1 = axes;
