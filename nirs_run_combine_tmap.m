function out = nirs_run_combine_tmap(job)
...
n_c = length(job.OtherNIRSmat);
OtherTOPO = cell(1,n_c);
for i0 = 1 : n_c
    [pth,nam] = spm_fileparts(job.OtherNIRSmat{i0,1});
    fileTOPO = fullfile(pth,'TOPO.mat');
    TOPOn = load(fileTOPO);
    for v1 = 2:5
        for h1 = 1:3
            for c1 = 1 : length(TOPOn.TOPO.v{v1}.g.hb{h1}.c)
                OtherTOPO{i0}.v{v1}.g.hb{h1}.c{c1}.Tmap = TOPOn.TOPO.v{v1}.g.hb{h1}.c{c1}.Tmap;
            end
        end
    end
    %OtherTOPO{i0} = TOPOn;
                ...
                if view_estimated
                    ...
                    for c1=1:nC %Loop over contrasts
                        ...
                        for h1=1:3 %including HbT
                            %Positive stats
                            [H,TOPO,big_TOPO] = combineTmap(H,TOPO,big_TOPO,OtherTOPO,v1,c1,h1,Z,W,F,CF,xCon,ns,1);
                            %Negative stats
                            [H,TOPO,big_TOPO] = combineTmap(H,TOPO,big_TOPO,OtherTOPO,v1,c1,h1,Z,W,F,CF,xCon,ns,0);
                        end
