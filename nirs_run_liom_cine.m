    ...
    try
        ...
        if ~isempty(NIRS) && (~isfield(NIRS.flags,'cine_OK') || job.force_redo)
            ...
            if run_cine_OK
                ...
                [CINE newDir fCINE] = nirs_load_CINE(newNIRSlocation);
                ...
                for v1=1:size(Z.views_to_run,2)
                    try
                        ...
                        if isempty(W.index_ch)
                            ...
                            disp(['No channel for view ' int2str(Z.views_to_run(v1)) ': Probable coregistration problem. Skipping this view']);
                        else
                            %rendering surface
