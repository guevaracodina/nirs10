...
fname = fullfile(dir0,[ftype '_' conc(1) '_Sess' int2str(f)]);
private_plot(d(1:NC/2,:),lp,fname)
fname = fullfile(dir0,[ftype '_' conc(2) '_Sess' int2str(f)]);
private_plot(d(NC/2+1:NC,:),lp,fname)

dt = 30;
lp2 = linspace(0,dt,round(dt*fs));
fname = fullfile(dir0,[ftype '_' conc(1) '_Sess' int2str(f) '_zoom']);
private_plot(d(1:NC/2,1:round(dt*fs)),lp2,fname)
fname = fullfile(dir0,[ftype '_' conc(2) '_Sess' int2str(f) '_zoom']);
