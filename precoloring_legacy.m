function [SPM] = precoloring_legacy(SPM, Y)
...
try
    ...
    elseif strcmp(nirs_data.cL.type, 'none') == 0 && strcmp(nirs_data.cH.type, 'none') == 1
        ...
        KY = spm_filter_HPF_LPF_WMDL_legacy(K_tmp, Y);
    ...
    SPM.nirs.KY = KY;
    %diagnostics
    %figure; k1=52; plot(SPM.nirs.trend(:,k1),'g'); hold on; plot(Y(:,k1),'b'); hold on; plot(KY(:,k1),'r'); hold off
end
clear nirs_data;

SPM.nirs.beta = SPM.xX.pKX * KY; % beta : least square estimate
res = spm_sp('r', SPM.xX.xKXs, KY); % Residuals 
SPM.nirs.ResSS = sum(res.^2); % Residual SSQ
clear KY; 
clear Y; 
clear res;
