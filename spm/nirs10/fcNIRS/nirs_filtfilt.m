function y = nirs_filtfilt(b,a,x)
%NIRS_FILTFILT Zero-phase forward and reverse digital IIR filtering.
%   Y = NIRS_FILTFILT(B, A, X) filters the data in vector X with the filter
...
%
%   Y = NIRS_FILTFILT(SOS, G, X) filters the data in vector X with the
...
%
%   Note that NIRS_FILTFILT should not be used when the intent of a filter is
