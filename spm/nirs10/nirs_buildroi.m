...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %         read_vertice = spm_orthviews('Pos',1);
        read_vertice = spm_orthviews('Pos');
        switch vertice
            case 'rf'
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %         bb = round(b);
        ...
        NS = NIRS.Cf.H.S.N;
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %         Pfp_rmv = NIRS.Cf.H.P.r.m.vx.fp; %%% ne pas garder
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %             Pfp_roi_rmv(:,i) = Pfp_rmv(:,Pkpt(i));%%%% a effacer
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %         bb = round(bb);
        ...
        marge =10;%10mm
        bb(1,1) = min(min(Pfp_roi_rmm(1,:))-marge,bb(1,1));
        bb(1,2) = min(min(Pfp_roi_rmm(2,:))-marge,bb(1,2));
        bb(2,1) = max(max(Pfp_roi_rmm(1,:))+marge,bb(2,1));
        bb(2,2) = max(max(Pfp_roi_rmm(2,:))+marge,bb(2,2));
        bb(1,3) = min(min(Pfp_roi_rmm(3,:))-marge,bb(1,3));
        bb(2,3) = max(max(Pfp_roi_rmm(3,:))+marge,bb(2,3));
        bb = round(bb);
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %         mat_roi2raw = eye(4) + [zeros(4,3) [bb(1,:)';0]];
        %%%%%%%%% maintenant bb est en mm, donc il faut le passer en vocels
        %%%%%%%%% pour pouvoir l'integrer dans la matrice V.mat
        bb = bb';
        for i=1:2
        bbvoxtemp(:,i) = V.mat\[bb(:,i);1];
        end
        bbvox = bbvoxtemp(1:3,:);
        %%% b(:,1) coin haut gauche : celui de l'origine en fait 0;0;0
        %%%
        % Translation : de l'image brute a la roi
        mat_roi2raw = eye(4) + [zeros(4,3) [bbvox(:,1);0]];
                
        V_roi.dim = [bbvox(1,2)-bbvox(1,1)+1 bbvox(2,2)-bbvox(2,1)+1 bbvox(3,2)-bbvox(3,1)+1];
        ...
        Y_roi = Y(bbvox(1,1):bbvox(1,2),bbvox(2,1):bbvox(2,2),bbvox(3,1):bbvox(3,2));
        ...
        for i=1:size(Pfp_roi_rmm,2)
            Pfp_roi_rmvtemp(:,i) = V_roi.mat\[Pfp_roi_rmm(:,i);1];
        end
