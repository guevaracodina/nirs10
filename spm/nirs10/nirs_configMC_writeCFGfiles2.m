    ...
    for iP = 1:NS+ND
        if isempty(Pvoid) || ~Pvoid(1,iP) %skip optodes with no data &&&'nS',iS,wl(iwl)
            ...
            if algo==2 % tMCimg
                ...
                % ID subject
                fprintf(fid, [';; Sujet # : ' char(job.G.n_b8i) '\n\n']);
                % ROI limits relative to whole MRI
                fprintf(fid, ';; ROI limits relative to whole MRI\n');
                fprintf(fid, [';; xmin : ' num2str(job.G.ROIlimits(1,2)) '\n']);
                fprintf(fid, [';; xmax : ' num2str(job.G.ROIlimits(2,2)) '\n']);
                fprintf(fid, [';; ymin : ' num2str(job.G.ROIlimits(1,1)) '\n']);
                fprintf(fid, [';; ymax : ' num2str(job.G.ROIlimits(2,1)) '\n']);
                fprintf(fid, [';; zmin : ' num2str(job.G.ROIlimits(1,3)) '\n']);
                fprintf(fid, [';; zmax : ' num2str(job.G.ROIlimits(2,3)) '\n\n']);
                % Number of photons
            ...
            elseif algo==1 % MCX
                ...
                end_time =  job.G.par.numTimeGates*job.G.par.deltaT;
                fprintf(fid,'%s %2.2e %1.2e   # time-gates(s): start, end, step\n','0.00e-9', end_time, job.G.par.deltaT);
                ...
                fprintf(fid,'%1.0f %3.0f %3.0f %3.0f            # x: voxel size, dim, start/end indices\n',...
                    job.G.par.voxelSize,job.G.ROIlimits(2,1),1,job.G.ROIlimits(2,1));
