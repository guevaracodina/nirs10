function nirs_focus_render(dat,brt,rend)
...
if ~isfinite(brt),%not used
    ...
    for i=1:length(rend),
        ...
        %mxmx = max(render_proj.all_focus{i}.map(:));
        %mnmn = min(render_proj.all_focus{i}.map(:));
        rend{i}.data{1} = render_proj.all_focus{i}.map;
