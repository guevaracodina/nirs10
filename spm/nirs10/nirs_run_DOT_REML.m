...
try
    ...
    if(size(X,1)<size(X,2)) %PP not a good test of rank?!
        ...
        %Set up the extended covariance model by concatenating the measurement
        ...
        for iter=1:maxIter
            ...
            % E-step:            
            %Bound lambda to avoid numerical prec. issues
    ...
    Beta= Xt_iCe_X\(Xt_iCe * Y);    
    
    Stats.tstat.beta=Beta;
    %C_beta_y = inv(Xt_iCe_X); %Impossibly long
    %Stats.tstat.Cov_beta=C_beta_y;
    Stats.tstat.dfe=size(X,2);
    Stats.tstat.t=Stats.tstat.beta.*sqrt(diag(Xt_iCe_X));
