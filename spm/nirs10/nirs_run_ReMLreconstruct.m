...
for Idx=1:size(job.NIRSmat,1)
    ...
    try
        ...
        Yb8i_c1 = find(YsegRR==1); % mask for GM
        Yb8i_c5 = find(YsegRR==5); % mask for skin
        ...
        %%% Y %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % for ifnirs=1:size()
        % fnirs = load(NIRS.Dt.fir.pp.p{ifnirs},'-mat');
        %
            ...
            switch job.WLruns
                case 1 % ON FAIT POUR LES 2 LONGUEURs D'ONDE
                    ...
                    switch job.ReML_method
                        ...
                        case 2 %%%%%%%%%% UTILISEEEEEEEEEEEEEEEEEEEEEEEEEEE
                            disp('peudo inverse');
                            meth = 'PInv';
                            ...
                            for iwl=1:2
                                %%% Tikhonov regularization : ancienne version
                                %                                 Ybar = Y_to{1,iwl};
                                %                                 X = Xwl{1,iwl};
                                %                                 Xbar = sparse(X*Msk);
                                %                                 alpha =1;
                                ...
                                %% Li et al extended Tikhonov regularization (est ce que ca a de l interet alors qu il va falloir trouver deux hyperparametres)
%                                 alpha =1;
% 
%                                 XX =Xbar'*Xbar;
%                                 YY = (Xbar'*Ybar);
%                                 clear Xbar;
%                                 XXLI = sparse(XX + alpha*eye(size(XX,2)));%eq.11 huppert_2010_hierarchical                                                   
%                                 Dmua{iwl} = XXLI \ YY;
                                
                                %% Interpretation bayesienne simple
                                % (regularization de Tikhonov avec des valeurs de covariances non nulles)
                    ...
                    spm_write_vol(V_O, betaR_HbO);%/max(max(max(betaR_HbO))));
                    ...
                    spm_write_vol(V_R, betaR_HbR);%/max(max(max(betaR_HbR))));
