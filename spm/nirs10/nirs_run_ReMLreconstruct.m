...
% m_c1(find(Yb8i==1))=1;
% m_c5 = zeros(size(Yb8i));
% m_c5(find(Yb8i==5))=1;
% clear Yb8i;


% % % % % % %
% % % % % % % jobR.image_in ={cs.b8i};
% % % % % % % out_b8i =  nirs_resize(jobR);
% % % % % % % Vb8i = spm_vol(out_b8i);
% % % % % % % Yb8iR = spm_read_vols(Vb8i);
% % % % % % % Yb8i = reshape(Yb8iR,[1 prod(Vb8i.dim)]);
% % % % % % % Nvx = prod(Vb8i.dim);
% % % % % % % % reperage des couches :
% % % % % % % Yb8i_c1 = find(Yb8i==1);
% % % % % % % Yb8i_c5 = find(Yb8i==5);
...
for itp=1:length(job.temp_pts)
    ...
    switch job.WLruns
        case 1 % ON FAIT POUR LES 2 LONGUEURs D'ONDE
            ...
            lst=(1:NC_cs);
            Qn=sparse(lst,lst,ones(size(lst)),NC_cs,NC_cs);
