...
for iSubj=1:size(job.NIRSmat,1)
    ...
    try
        ...
        if ~isempty(NIRS) && (~isfield(NIRS.flags,'PVE_OK') || job.force_redo)
            NIRS.Dt.fir.DPF =[];
            NIRS.Dt.fir.PVE =[];
            ...
            for Ci=1:NIRS.Cf.H.C.N
                try
                    ...
                    disp(exception.identifier);
                    disp(exception.stack(1));% le fichier d histoire n a pas ete trouve...
        ...
        disp(exception.identifier);
        disp(exception.stack(1));
        disp(['Calculus of PVE failed for subject ' int2str(iSubj) ' for ' job.NIRSmat{iSubj,1}]);
    end
end
