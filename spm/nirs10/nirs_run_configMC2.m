...
for Idx=1:size(job.NIRSmat,1)
    try
        ...
        if ~isempty(NIRS) && (~isfield(NIRS.flags,'configMC_OK') || job.force_redo)
            ...
            NIRS.flags.configMC_OK = 1;       
            save(job.NIRSmat{Idx,1},'NIRS');
        end
    catch exception
        disp(exception.identifier);
        disp(exception.stack(1));
        disp(['Could not run MonteCarlo configuration for subject' int2str(Idx) ' for ' job.NIRSmat{Idx,1}]);
    end
end
