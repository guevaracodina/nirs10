...
if FFX || size(job.NIRSmat,1)==1
    ...
    for Idx=1:ns
        ...
        if Idx==1
        %A new folder is created at each group analysis
        dir_ga = [dir1 '\' 'Group_' int2str(ns) 'subjs_session' int2str(session_n)];
        mkdir(dir_ga);
        end
    ...
    try
            ...
            if view_estimated
                ...
                for h1=1:2 %exclude HbT for now
                    ...
                    for c1=1:length(xCon)
                        ...
                        erdf_group = max(erdf_group(:)); %quick fix...
                        nirs_draw_figure(4,brain,tmap_group,info1,...
                            info_for_fig1,split,dir_ga,erdf_group,[],p_value,gen_fig,gen_tiff)          %info_for_fig1,split,dir1,erdf_group,[],p_value,gen_fig,gen_tiff)
                        ...
                        erdf_group = max(erdf_group(:)); %quick fix...
                        nirs_draw_figure(4,brain,tmap_group,info1,...
                            info_for_fig1,split,dir_ga,erdf_group,[],p_value,gen_fig,gen_tiff)
        ...
        ftopo = fullfile(dir_ga,'TOPO.mat');%CB; dir0
        save(ftopo,'TOPO');
    catch
