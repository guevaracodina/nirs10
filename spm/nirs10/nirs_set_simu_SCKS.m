function M = nirs_set_simu_SCKS(M,it1)
%Buxton-Friston balloon and other models
M.P = M.S.pA(it1,:)';
%efficacies
%M.P(end-size(M.U.u,2)+M.S.simuS) = 1;
%which dt is used in there?
ys = feval(M.EM.spm_integrator,M.P,M,M.U); %PP this needs to change
...
if M.LPF.lpf_gauss_On || M.S.simuUpsample > 1
     K = get_K(1:size(Y.y,1),M.LPF.fwhm1,Y.dt); %probably needs to change
     %filter forward
