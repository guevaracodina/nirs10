...
%   $Revision: 1.23.4.35 $  $Date: 2010/08/23 23:10:52 $
%=============================================================================
rawData = {};
Sheet1 = 1;
if nargin < 2
...
if strcmpi(mode, 'basic') 
    warning(message('MATLAB:xlsread:Mode'));
        basicMode = true;
else
    try
...
%     aTemp = Excel.workbooks.Add(); aTemp.Close();
    ...
    if nargin >= 2
        ...
        if ~isequal(sheet,-1)
            ...
            try % importing a data range.
                ...
                error(message('MATLAB:xlsread:RangeSelection'));
            end
    ...
    try
        ...
        %Do not pollute last error state
    end
...
if isempty(data)
    ...
    lastrow = nrows;
    while (lastrow>=firstrow && all(emptycells(lastrow,:)))
        lastrow = lastrow-1;
    end
    
    % find start of trailing NaN rows
    warnstate = warning('off', 'MATLAB:nonIntegerTruncatedInConversionToChar');
    TrimTrailingNaNs(1024);
    TrimTrailingNaNs(1);
    warning(warnstate);
    % remove trailing NaN rows    
    data=data(firstrow:lastrow,:);
    emptycells = emptycells(firstrow:lastrow,:);
    [n,m] = size(data);
    textArray = cell(size(data));
    textArray(:) = {''};
end

%Helper function to trim trailing rows
    function TrimTrailingNaNs(stride)
        while lastrow>=firstrow + stride - 1 
            dataSection =  data(lastrow - (stride-1):lastrow,:);
            if ~all(cellfun(@isnumeric,dataSection(:)))           
                break;
            elseif ~all(isnan([dataSection{:}]))
                break;
            end            
            lastrow = lastrow-stride;
        end        
    end
...
vIsNaN = emptycells | strcmpi(data,'nan') | vIsText;
if any(vIsNaN(:))
    data(vIsNaN)={NaN};
end
...
if nargin == 1 || isempty(sheet)
    ...
    try
        ...
        err = MException('MATLAB:xlsread:WorksheetNotOpened',...
                         'XLSREAD unable to read sheet %s.\n%s',...
                           sheet,exception.message);
        throw(err);
    end
end
