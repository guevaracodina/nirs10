function [X,Xname,Fc] = spm_Volterra_third(U,bf,V)
% generalized convolution of inputs (U) with basis set (bf)
% FORMAT [X,Xname,Fc] = spm_Volterra(U,bf,V);
% U          -  input structure array
% bf         -  Basis functions
% V          -  [1, 2 or 3] order of Volterra expansion [default = 1]
%
% X          -  Design Matrix
...
for i = 1:length(U)          %Loop over scans
    ind   = [];
    for k = 1:size(U(i).u,2) %Loop over onset types
