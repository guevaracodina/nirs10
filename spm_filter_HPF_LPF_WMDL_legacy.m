function [argout] = spm_filter_HPF_LPF_WMDL_legacy(K,Y)
...
if nargin == 1 && isstruct(K)
    ...
    if isstruct(K)
           ...
           for s = 1:length(K)
            ...
            switch K(s).HParam.type
                case 'Wavelet-MDL'
%                     [biasM betaM errM] = detrend_wave_rev_v03(full(y), K(s).X(:,1:end-1), K(s).HParam.M);
                    % we set the number of trials for wavelet-MDL
                    % detrending as 4 %PP Will work only for one session,
                    % due to removing constant regressor in design matrix
                    K(s).HParam.wavelet_depth = 9; %quick fix
                    [biasM betaM errM] = detrend_wave_rev_v03(full(y), K(s).X(:,1:end-1), K(s).HParam.M, K(s).HParam.wavelet_depth);
                    y = y - biasM;
                    %Y(K(s).row,:) = y;
                    %argout = [y biasM betaM errM];                      
                    %return
                case 'DCT'
                    y = y - K(s).X0*(K(s).X0'*y);                    
            end
